...
class MyCalendarTwo {
    ...
    func book(_ start: Int, _ end: Int) -> Bool {
        for i in 0 ..< doubleBookedIntervals.count {
            let interval = doubleBookedIntervals[i]
        ...
        for i in 0 ..< events.count {
            ...
            if !(end <= event[0] || start >= event[1]) {
                doubleBookedIntervals.append([max(start, event[0]), min(end, event[1])])
            }
        }
