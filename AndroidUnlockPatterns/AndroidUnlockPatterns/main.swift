...
//
//  Created by Behzad Dogahe on 7/29/21.
//
...
func dfs(_ visited: inout [Bool], _ skip: [[Int]], _ curr: Int, _ rem: Int) -> Int {
    if rem < 0 {
        return 0
    }
    if rem == 0 {
    ...
    for i in 1 ... 9 {
        if !visited[i] && (skip[curr][i] == 0 || visited[skip[curr][i]]) {
            result += dfs(&visited, skip, i, rem - 1)
        }
    }
    visited[curr] = false
    return result
...
func numberOfPatterns(_ m: Int, _ n: Int) -> Int {
    var skip: [[Int]] = Array(repeating: Array(repeating: 0, count: 10), count: 10)
    skip[1][3] = 2
    skip[3][1] = 2
    skip[1][7] = 4
    skip[7][1] = 4
